...
import LogoImg from "../../app/login.png";
...
export function Navbar() {
  ...
  const handleNavigationClick = (clickedItemName) => {
    const updatedNavigation = navigation.map((itemObj) => {
      console.log("Onclick Clicked");
      if (itemObj.name === clickedItemName) {
        return { ...itemObj, current: true }; // Update the 'current' property for the clicked item
      } else {
        return { ...itemObj, current: false }; // Reset 'current' property for other items
      }
    });
                  ...
                  <div className="flex space-x-4">
                    {navigation.map((item) => (
                      <a
                        ...
                        href={item.href}
                        onClick={() => {
                          return handleNavigationClick(item.name);
                        }}
